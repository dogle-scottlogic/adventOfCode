...
public class DayTwoPartTwo implements PuzzleDay {
    ...
    private Integer[] getNounAndVerb(String input) {
        IntcodeProgram intcodeProgram = new IntcodeProgram();
        String [] values = input.split(",");
        String goal = "19690720";
        int noun = 0;
        int verb = 0;

        while (verb <= 99) {
            values[1] = String.valueOf(noun);
            values[2] = String.valueOf(verb);

            String[] testResults = intcodeProgram.runProgram(values);

            if (testResults[0].equals(goal)) {
    ...
    public void solvePuzzle() {
        ...
        Integer[] nounAndVerb = getNounAndVerb(input);

        if (nounAndVerb == null) {
            System.out.println("Not found");
        } else {
            Integer result = 100 * nounAndVerb[0] + nounAndVerb[1];
